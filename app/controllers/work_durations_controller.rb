class WorkDurationsController < ApplicationController
  ...
  def weekly_wise_data
    ...
    @work_durations = all_work_durations["#{params[:date]}"]
    @date = params[:date]
    @work_duration = WorkDuration.new
  ...
  def weekly_update
    ...
    (today.at_beginning_of_week..today.at_end_of_week).map.each_with_index do |day, index|
      ...
      # extract the number of hours for each
        @work_duration.hours = params[["a","b","c","d","e","f","g"][index]+@work_duration.id.to_s].to_i 
        @work_duration.time_sheet_status = "pending"
  ...
  def create
    ...
    for vendor in employee.vendors
      ...
      (date.at_beginning_of_week..date.at_end_of_week).to_a.take(5).map.each_with_index do |day, index|
        # extract the number of hours for each 
        hours_worked = params["work_duration"][["a","b","c","d","e"][index]+vendor.id.to_s]
        ...
        if @work_duration == nil
          @work_duration = project.work_durations.new(hours:hours_worked, work_day:day, time_card_certify:time_card_certify,company_certify:company_certify,save_for_later: save_for_later)
        else
        ...
        if !@work_duration.save
          @errors << @work_duration.errors.full_messages
        end
      end
    end
    ...
    respond_to do |format|
      if !@errors.present?
        ...
        format.html { redirect_to weekly_wise_data_work_durations_path(eid:employee.id,date:date.strftime("%B %d, %Y")) }
        format.json { render json: @work_duration.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  private
    ...
    def work_duration_params
      params.require(:work_duration).permit(:hours, :work_day, :employee_id,:time_card_certify,:company_certify,:save_for_later)
    end
end
